...
const CompleteTask = ({ data, employeeId, onTaskUpdate }) => {
  ...
  return (
    <div className="h-60 w-[350px] shrink-0 bg-white shadow-lg rounded-2xl p-6 flex flex-col justify-between cursor-pointer hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
      ...
      >
        ...
        <span className="text-gray-500 text-xs font-medium">{data.taskDate}</span>
      </div>

      <div className="mt-6">
        <h2 className="text-xl font-bold text-gray-800">{data.taskTitle}</h2>
        <p className="text-gray-500 mt-2 text-sm">{data.taskDescription}</p>
      </div>

      <div className="flex justify-between items-center mt-6">
        <span className="text-xs text-green-500 font-semibold bg-green-50 px-2 py-1 rounded-full">✓ Completed</span>
        <button
